    ...
    @foreach ($warehouses  as $warehouse )
      <td > <p class="fw-normal ms-2">{{$i++}}</p></td>
        ...
        <td >
          <div class="d-flex">
          @can('view-warehouse')
          <a href="{{ (isset($branch) && $branch) ? '/branchs/'.$branch.'/warehouses/'.$warehouse->id.'/show': '/warehouses/'.$warehouse->id.'/show' }}" class="  rounded btn  btn-warning px-2 pb-1 pt-1 mr-2 " >View</a>
          @endcan
          @can('edit-warehouse')
